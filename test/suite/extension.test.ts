...
import { match } from '../../src/gotoTests';
import { Rule, DEFAULT_RULES } from '../../src/rule';
...
suite('match', () => {
  ...
  test('match replacement 1', () => {
    assert.equal(match('in', { pattern: '(in)', replacement: '$1.test' }), 'in.test');
  });

  test('match replacement 2', () => {
    assert.equal(match('ab', { pattern: 'a(b)', replacement: '$1.test' }), 'b.test');
  });
...
suite('Ruby', () => {
  ...
  test('minitest 1', () => {
    transitive('app/controllers/a/file_controller.rb', 'test/integration/a/file_controller.rb');
  });
...
suite('Go', () => {
  test('_test.go', () => {
    transitive('/home/brian/kubernetes/pkg/kubectl/cmd/cmd.go', '/home/brian/kubernetes/pkg/kubectl/cmd/cmd_test.go');
  });
});
